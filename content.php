		...
		<div class="box-social-media ac-sb">
			<ul class="social-media row">
				<li class="hidden-lg-up col-xs-4">
					<a class="svg-i i-wa-white" href="whatsapp://send?text=Queria compartilhar isso com você: <?php esc_url(the_permalink()) ?>" title="<?php _e( 'Compartilhar no whatsapp', 'odin' ); ?>"><?php _e( 'Compartilhar', 'odin' ); ?></a>
				</li>
				<li class="col-xs-4">
				...
				<li class="col-xs-4">
					<!-- <a class="svg-i i-tt-c" href="#" title="<?php //_e( 'Compartilhar no twitter', 'odin' ); ?>"></a> -->
...
<?php // Remove AstroshopSense If Cartomancia & Videncia  ?>
<?php if ( $category[0]->cat_ID == 8 || $category[0]->cat_ID == 3 || $category[0]->cat_ID == 10 || $category[0]->cat_ID == 13) : ?>

<?php else: ?>
 <div id="astroshoppingsense_below_article" class="wengosense-cached ac-sb wg-list">
 	<h3><?php _e( 'Ofertas para você', 'odin' ); ?></h3>
 	<?php
 		include_once get_template_directory() . '/cron/wengosense_cached.php';
 		echo get_astroshopping_products(array('zone'=>'article'));
 	 ?>
  </div>
<?php endif; ?>
